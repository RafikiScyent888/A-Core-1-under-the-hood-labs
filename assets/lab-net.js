...
const TERM_OPTS = [
  ...
  /* FIVE MORE, AND THE MODEL IS WHY THEY CAN BE ASKED.

     The resting machine in this lab is a wall outlet, and until the
     machines got named parts it was three things a student could point
     at: a plate, a jack and its contacts. It is eight now — the wall, the
     backbox, the faceplate, the port labels, the BLANK in the second
     port, the keystone, the contacts and the patch lead lying beside it —
     and each of these five faults lives on one of the five that were not
     there before.

     That is the good kind of five-more, and it is the same argument the
     printer's feed faults were added under: before the photograph, a
     student sent to look at a feed fault was sent to look at one grey
     cylinder. Before these parts, a student sent to look at an outlet was
     sent to look at a white rectangle. */
  { key: "blank",   label: "There is no jack in that port at all — it is a blank insert" },
  { key: "label",   label: "The port label does not match anything on the patch panel" },
  { key: "solid",   label: "Solid core has been crimped into plugs and used as a patch lead" },
  { key: "bend",    label: "The cable is kinked behind the jack — the backbox is too shallow" },
  { key: "contact", label: "A contact in the JACK is corroded — the plug is fine" }
];
...
const TERM_CASES = [
  ...
  /* THE FIVE THE OUTLET EARNED, and every one of them is a fault at the
     WALL rather than in the plug — which is the half of a structured
     cabling run this stage could not previously ask about.

     They are near misses of each other on purpose. Four of the five
     present as "this data point does not work properly", and the
     discriminator is buried in the report every time: whether the port
     was ever alive, whether the fault moves with the lead, whether it
     comes and goes when something is touched, and whether the tester
     failed at one END or along the RUN. That is the "reading the
     scenario" gap, deliberately. */
  { key: "blank", said: "The desk has a two-gang plate and the right-hand port has never " +
      "worked. Looking into it there is no throat and no gold at all, and the plate is the " +
      "same white all the way through where the opening should be.",
    tell: "IT IS A BLANK, not a dead port. Somebody ran ONE cable and fitted a two-gang plate, " +
      "which is normal and cheap — a blank insert is the colour of the plate precisely so you " +
      "can see from across the room that nothing was ever run to it. There is nothing to fix " +
      "at this end: it is a new cable pull or it is nothing." },
  { key: "label", said: "The port is live and fast. The engineer needs to move it to another " +
      "VLAN and the label on the plate reads B14; there is no B14 anywhere on the patch panel, " +
      "and the panel's own labels run A1 to A24.",
    tell: "The LABEL does not match the panel. Nothing is wrong electrically — what is broken " +
      "is the thing that makes an outlet part of a system rather than a hole in a wall. " +
      "Without a label that matches, the only way to find the other end is to tone it, and the " +
      "fix is to tone it once and relabel BOTH ends." },
  { key: "solid", said: "The lead from the outlet to the PC was made on site. It works, and it " +
      "drops every time the desk is wheeled out for cleaning, then comes back when somebody " +
      "wiggles it. The conductors inside the plug are single strands, not bundles.",
    tell: "SOLID CORE used as a patch lead. Solid is made to be punched down once and never " +
      "moved; it work-hardens and fractures inside the plug where you cannot see it. A patch " +
      "lead is STRANDED and crimped, and the two are not interchangeable even though the plug " +
      "fits both." },
  { key: "bend", said: "A brand new run, certified at the panel end and failing at the outlet " +
      "end on NEXT. The jack is in a 25 mm box and the installer had to fold the cable back on " +
      "itself to get the keystone to clip in.",
    tell: "The cable is KINKED behind the jack. A bend tighter than about four times the " +
      "cable's diameter changes the spacing inside the pairs, and that shows up as crosstalk " +
      "at the end where the bend is. Nothing is wrong with the termination — the BACKBOX is " +
      "too shallow for the cable that went into it." },
  { key: "contact", said: "One desk. Three different PCs, three different leads, same result: " +
      "the link comes and goes when anybody knocks the plug. Along the throat of the jack one " +
      "of the eight gold contacts is dull and green where the other seven are bright.",
    tell: "A corroded contact IN THE JACK. Three leads ruling themselves out is the whole clue " +
      "— if the fault stays when the lead changes, the fault is not in the lead. Corrosion " +
      "makes an intermittent rather than an open, which is the worst kind of call to be sent " +
      "to, and the keystone is a five-minute replacement." }
];
...
const SITES = [
  ...
  /* FIVE MORE SITES, asked for by the named-scenario picker.

     This lab had five, measured across 400 seeds, so a list of six could
     not be filled. The staff counts and floor counts are what the sizing
     questions read, so each of these is a genuinely different sum rather
     than a new name on the same numbers — and the spread now runs from a
     four-person practice to a fifty-seat office, which is the range the
     link and switch questions are actually about. */
  { name: "a four-chair barber shop with a booking tablet", staff: 4, floors: 1 },
  { name: "a village pharmacy with a dispensing robot", staff: 7, floors: 2 },
  { name: "an independent estate agency", staff: 16, floors: 3 },
  { name: "a light engineering workshop and its office", staff: 24, floors: 1 },
  { name: "a regional insurance broker", staff: 48, floors: 2 }
];
    ...
    return {
      ...
      questions: [{
        ...
        hints: [
          ...
          /* RUNG 2 IS A PRINCIPLE, STATED GENERALLY, WITH THE CASE LEFT
             TO THEM — and it had to be rewritten when the outlet's five
             faults went in. The old one was about continuity against
             pairing, which decides the plug faults and says nothing at
             all about a corroded jack or a kinked cable behind it. A
             rung that only covers half the pool is a rung that goes
             silent exactly when a student is stuck. */
          "Ask WHERE the fault is before asking what it is. A fault that travels with the LEAD " +
            "is in the lead; one that stays when the lead is changed is at the wall or in the " +
            "run. The report has almost always already told you which — in what was swapped, " +
            "or in which end the tester failed at.",
          /* RUNG 3 NARROWS BY FAMILY, not by naming options. The pool is
             thirteen deep and the six on screen change with the seed, so
             a rung that struck options by name would strike things that
             are not there — which is worse than silence. Three families,
             each with a reason, and it always leaves more than two
             alive. */
          "Sort them into three piles and most of the list goes. Some are IN THE PLUG, which " +
            "you cut off and redo. Some are AT THE WALL — the jack, the plate, the box behind " +
            "it — which the plug end cannot fix. And some are not termination faults at all: " +
            "the length of the run, the shield, the label. If the symptom shows under load, " +
            "over distance, or only when something is touched, the crimp may be innocent."
        ],
...
export function selfCheck(sc) {
  ...
  /* EVERY CASE'S ANSWER HAS TO BE IN THE POOL IT IS OFFERED FROM.

     `fromPool` builds the six options by marking the one option whose key
     matches the case. A case naming a key that is not in its pool
     produces a question with SIX WRONG ANSWERS — it renders, it grades,
     and every student who reads it correctly is told they are wrong.
     That exact class of bug has been found three times in this build and
     never by looking at a render.

     It is worth having now rather than before because the pools have
     started to GROW: the termination pool went from eight to thirteen
     when the wall outlet got named parts, and a typo in one of those five
     keys would have been invisible. */
  [["wall", WALL_CASES, WALL_OPTS], ["fibre", FIBRE_CASES, FIBRE_OPTS],
   ["termination", TERM_CASES, TERM_OPTS]].forEach(function (t) {
    const [name, cases, pool] = t;
    cases.forEach(function (c, i) {
      ok(pool.some(function (o) { return o.key === c.key; }),
         name + " case " + i + " answers \"" + c.key + "\", which is not in its option pool — " +
         "every option offered would be wrong");
      ok(!!c.tell && !!c.said, name + " case " + i + " is missing its report or its answer");
    });
    /* And the pool must still be able to fill six. */
    ok(pool.length >= 6, name + " has only " + pool.length + " options, and every question " +
       "in this build offers six");
  });
