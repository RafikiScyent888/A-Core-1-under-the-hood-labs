...
/* `within` scopes the measurement to one subtree.

   It exists for ONE situation and it is not a way of quieting failures:
   a MODAL over a scrim. The PIN dialog paints rgba(0,0,0,0.62) across the
   whole page, so every run of text behind it measures against a darkened
   version of its own background — the light theme's body text came back
   at 1.38:1 and the heading at 2.74:1. That content is deliberately
   obscured and inert; it is not text a student is being asked to read.
   Measuring it is the checker being wrong about what is on screen, which
   is the same class of mistake as reading the cascade instead of the
   pixels.
...

{
  ...
  /* BOTH THEMES, AND THAT DOUBLING IS THE POINT OF THE LIGHT THEME
     EXISTING AT ALL.

     Adding a second theme is five minutes of tokens and a day of proving
     them. A light palette that is merely "not obviously broken" hands
     half of these students a page they cannot read, and does it quietly:
     a washed grey on white measures far worse than it looks, and the
     eye is a poor detector for the difference between 4:1 and 7:1.

     The theme is set through the same stored key the page uses, before
     the reload, so what is measured is what a student who flipped the
     switch actually gets — not a class toggled on by the harness. */
  for (const lab of labs) {
    for (const len of ["quick", "layered", "project"]) {
      for (const theme of ["dark", "light"]) {
        const { p: q } = await page();
        await q.evaluate((t) => localStorage.setItem("uthl.theme.v1", t), theme);
        await q.reload();
        const got = await q.evaluate(() => document.documentElement.getAttribute("data-theme"));
        if (got !== theme) {
          fails.push(`${lab}/${len}: asked for the ${theme} theme and the page came back "${got}" — ` +
                     `the rest of this row measured nothing useful`);
          await q.close();
          continue;
        }
        ...
        r.filter((x) => !x.ok).forEach((x) =>
          fails.push(`${theme} ${lab}/${len} ${x.got}:1 needs ${x.need} at ${x.px}px "${x.text.slice(0, 28)}"`));
        await q.close();
      }
    }
  }
  record("contrast", fails.length === 0,
    fails.length ? fails.slice(0, 6).join(" | ")
      : `${measured} text runs across ${labs.length * 3} views in BOTH themes, all AAA`);
  await p.close();
}

/* --- 3b. THE SURFACES THE SWEEP ABOVE CANNOT REACH -------------------
   The sweep walks tabs and lengths, which is every view a student can
   get to by clicking. Four surfaces are not reachable that way and were
   therefore never measured at all:

     the finished shelf   drawn only once something has been finished
     the parked shelf     drawn only once something has been put down
     the PIN dialog       behind a button, and it is a modal over a scrim
     the answer panel     behind the PIN, inside a running lab

   A view nothing drives is a view with no contrast floor, and "it uses
   the same tokens as everything else" is exactly the reasoning that let
   a gradient page report white on white. These are set up rather than
   waited for: the two shelves are seeded straight into storage, and the
   PIN is typed.

   Both themes, same as above, and for the same reason. */
{
  const fails = []; let measured = 0, views = 0;
  for (const theme of ["dark", "light"]) {
    const { p: q } = await page();
    await q.evaluate((t) => {
      localStorage.setItem("uthl.theme.v1", t);
      /* A job on each shelf. Written straight in rather than played
         through the UI: this check is about whether the rows can be
         READ, and verify/parked.mjs and verify/finished.mjs already
         prove they get there honestly. */
      localStorage.setItem("uthl.parked.v1", JSON.stringify({
        raid: { lab: "raid", seed: 481203, length: "project", at: 2, stages: 9,
                stageTitle: "Populate the array", answered: { "rd-cap": true },
                attempts: {}, parkedAt: Date.now() - 7200000 } }));
      localStorage.setItem("uthl.finished.v1", JSON.stringify({
        "power/700205": { lab: "power", seed: 700205, name: "Sentinel Data Services",
          length: "lab", right: 9, total: 11, runs: 2,
          firstAt: Date.now() - 900000, finishedAt: Date.now() - 300000 } }));
    }, theme);
    await q.reload();
    await q.waitForTimeout(400);

    /* the two shelves */
    let r = await contrast(q);
    measured += r.length; views++;
    r.filter((x) => !x.ok).forEach((x) =>
      fails.push(`${theme} shelves ${x.got}:1 needs ${x.need} at ${x.px}px "${x.text.slice(0, 30)}"`));

    /* the PIN dialog, open, over its scrim */
    await q.click("#instructorBtn");
    await q.waitForSelector(".pin-box", { timeout: 5000 });
    /* WRONG FIRST, so the refusal line is on screen when it is measured.
       An error nobody can read is the one that matters most. */
    await q.fill("#pin-in", "0000");
    await q.click(".pin-acts .btn.primary");
    await q.waitForTimeout(80);
    /* THE DIALOG ONLY. See the note on `within` — the scrim darkens the
       whole page behind it, and that page is not what anybody is being
       asked to read while a modal is open. */
    r = await contrast(q, ".pin-box");
    if (!r.length) fails.push(`${theme}: the PIN dialog painted no text to measure`);
    measured += r.length; views++;
    r.filter((x) => !x.ok).forEach((x) =>
      fails.push(`${theme} PIN ${x.got}:1 needs ${x.need} at ${x.px}px "${x.text.slice(0, 30)}"`));

    /* unlocked: the job sheet, which loads all eight lab modules */
    await q.fill("#pin-in", "3693");
    await q.click(".pin-acts .btn.primary");
    await q.waitForFunction(() => document.querySelectorAll("#insSheet .ins-jobs").length >= 8,
      { timeout: 90000 }).catch(() => {});
    r = await contrast(q);
    measured += r.length; views++;
    r.filter((x) => !x.ok).forEach((x) =>
      fails.push(`${theme} job sheet ${x.got}:1 needs ${x.need} at ${x.px}px "${x.text.slice(0, 30)}"`));

    /* and the answer panel, inside a running lab */
    await q.click("#tab-raid");
    await q.selectOption("#length", "quick");
    await q.click(".btn.start");
    await q.waitForFunction(() => window.__UTHL && window.__UTHL.running(), { timeout: 20000 });
    await q.waitForSelector(".truth", { timeout: 8000 }).catch(() => {});
    r = await contrast(q);
    measured += r.length; views++;
    r.filter((x) => !x.ok).forEach((x) =>
      fails.push(`${theme} answers ${x.got}:1 needs ${x.need} at ${x.px}px "${x.text.slice(0, 30)}"`));
    await q.close();
  }
  record("contrast off the beaten path", fails.length === 0,
    fails.length ? fails.slice(0, 6).join(" | ")
      : `${measured} text runs across ${views} views the tab sweep cannot reach ` +
        `(both shelves, the PIN prompt with its refusal, the job sheet, the answer panel), all AAA`);
}
...
for (const LAB of BUILT) {
  const trouble = [];
  let measured = 0;
  /* BOTH THEMES HERE TOO, and this is the sweep that matters most: the
     plan page is a paragraph and a dropdown, the RUNNING lab is where a
     student spends an hour. Quick in dark and Project in light covers
     every stage of the lab across both themes without doubling a run
     that already walks every stage twice — the stages are the same
     markup either length, and what changes between the two themes is the
     palette rather than the content. */
  for (const [len, theme] of [["quick", "dark"], ["project", "light"]]) {
    const { p } = await page();
    await p.evaluate((t) => localStorage.setItem("uthl.theme.v1", t), theme);
    await p.reload();
    const got = await p.evaluate(() => document.documentElement.getAttribute("data-theme"));
    if (got !== theme) {
      trouble.push(`${LAB}/${len}: asked for the ${theme} theme and got "${got}"`);
      await p.close();
      continue;
    }
    await p.click("#tab-" + LAB);
    ...
    for (let i = 0; i < total; i++) {
      ...
      measured += r.length;
      r.filter((x) => !x.ok).forEach((x) =>
        trouble.push(`${theme} ${LAB}/${len} stage ${i} ${x.got}:1 needs ${x.need} at ${x.px}px "${x.text.slice(0, 26)}"`));
    }
    await p.close();
  }
  record(LAB + " contrast", trouble.length === 0,
    trouble.length ? trouble.slice(0, 5).join(" | ")
      : `${measured} text runs inside the running lab, dark and light, all AAA`);
}
  ...
  for (const LAB of WITH_BENCH) {
    ...
    for (let i = 0; i < total && !got; i++) {
      ...
      /* WAIT FOR THE CONDITION, NOT FOR A GUESS AT HOW LONG IT TAKES.
         ...
         `:not([data-resting])` skips the MODEL DOCK when it is showing
         the lab's resting machine rather than a bench. Both are
         `.panel-bench`; only one of them is this lab's own hardware with
         its own controls, and that is what this check is about. Without
         it, three labs whose first stage is a brief were measured on a
         one-control resting machine. */
      await p.waitForFunction(() => {
        const box = document.querySelector(".panel-bench:not([data-resting])");
...
[["wear framing", "wear-frame.mjs", []],
 ...
 /* THE MODEL DOCK. Two separate claims and they need two checks.

    `dock persists` walks every stage of every lab and holds BOTH that a
    machine is on screen and that the whole walk costs ONE WebGL context.
    A dock that remounted per stage would satisfy the first and defeat the
    point of the second — which is the "written and reachable are
    different claims" trap in its newest clothes.

    `machine framing` sweeps the eleven resting machines at the canvas
    widths the dock really hands out, on BOTH axes. Two formulas for that
    frame were wrong before one was right, and neither was caught by
    looking at a render. */
 ["dock persists", "dock-persists.mjs", []],
 ["dock persists calibrated", "dock-persists.mjs", ["--calibrate"]],
 ["machine framing", "machine-frame.mjs", []],
 ["machine framing calibrated", "machine-frame.mjs", ["--calibrate"]],
 /* REAL DEVICE PROFILES, not a desktop window squeezed narrow — a narrow
    desktop window has a mouse, no touch, and a device pixel ratio of 1.
    Students do this as homework, alone, on whatever they have. It found
    two things on its first run: a select that pushed a 320px phone
    sideways, and a 37px tap target on every device. */
 ["devices", "devices.mjs", []],
 /* THE NAMED JOBS. Renders six, preselects the first, and - the part
    worth checking - the seed the student picked is the seed the lab
    actually runs. A picker that renders and does not change the scenario
    is the worst kind of working. */
 ["named jobs", "jobs.mjs", []],
 /* THE STORAGE ROOM. The claim is not that a button exists - it is that
    the job which comes back is THE SAME JOB: same seed, same stage, the
    questions already answered still answered, and the hint ladder still
    where it was. A shelf that loses a student's place is worse than no
    shelf, because they will have trusted it with an hour. */
 ["storage room", "parked.mjs", []],
 ["storage room calibrated", "parked.mjs", ["--calibrate"]],
 /* THE FINISHED SHELF. Four claims, and one of them is a bug this found
    rather than a feature it guards: finishing a job that had been PARKED
    left the half-done copy on the shelf for ever, inviting the student
    back into work they had already completed. The two shelves are one
    job in two states and only one can be true at a time. */
 ["finished shelf", "finished.mjs", []],
 ["finished shelf calibrated", "finished.mjs", ["--calibrate"]],
 /* THE ELEVEN MACHINES HAVE NAMED PARTS AND A STUDENT REACHES THEM. The
    module arm reads the tables; the browser arm drives eight real labs
    and counts the controls the dock actually offers, because the dock
    used to offer exactly one on every machine in the build. */
 ["machine parts", "machine-parts.mjs", []],
 ["machine parts calibrated", "machine-parts.mjs", ["--calibrate"]],
 /* BORDERS, WHICH THE CONTRAST SWEEP CANNOT SEE. A blind spot written
    down in the stylesheet itself — "borders are held by eye and by these
    definitions, not by the sweep" — and held by eye is not held. It
    found three feedback boxes whose outlines measured under 2:1 against
    the panel they sat on. */
 ["borders", "borders.mjs", []],
 ["borders calibrated", "borders.mjs", ["--calibrate"]]]
